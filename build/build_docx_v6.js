...
function inline(text, base = {}) {
  ...
  for (const p of parts) {
    if (/^\[\^\d+\]$/.test(p)) runs.push(new TextRun({ text: p.slice(2, -1), superScript: true, ...base }));
    else if (/^\*\*[^*]+\*\*$/.test(p)) runs.push(new TextRun({ text: p.slice(2, -2), bold: true, ...base }));
...
const tableList = [];
const boxList = [];
let abstract = [], resume = [], takeaways = [], ack = [], abbr = [], exec_ = [], kwLines = [];
const refEntries = [];
const noteDefs = [];
...

{
  ...
  const flushPara = () => {
    ...
    paraBuf = [];
    if (!text) return;
    if (mode === 'abstract') { if (/^(Keywords|JEL codes):/.test(text)) kwLines.push(text); else abstract.push(text); }
    else if (mode === 'resume') resume.push(text);
    else if (mode === 'ack') ack.push(text);
    else if (mode === 'exec') exec_.push(text);
    else if (mode === 'notes') { const m = text.match(/^\[\^(\d+)\]:\s*(.*)$/s); if (m) noteDefs.push({ n: m[1], text: m[2] }); else if (noteDefs.length) noteDefs[noteDefs.length-1].text += ' ' + text; }
  ...
  for (; i < paperMd.length; i++) {
    ...
    if (t === '# References') { flushPara(); flushTable(); mode = 'refs'; continue; }
    if (t === '# Notes') { flushPara(); mode = 'notes'; continue; }
    if (t === '# Acknowledgements') { flushPara(); mode = 'ack'; continue; }
    if (t === '# Abbreviations and acronyms') { flushPara(); mode = 'abbr'; continue; }
    if (t === '# Executive summary') { flushPara(); mode = 'exec'; continue; }
    ...
    if (/^# Key (take-aways|messages)/i.test(t)) { flushPara(); mode = 'take'; continue; }
    ...
    if (/^- /.test(t)) {
      ...
      else if (mode === 'abbr') abbr.push(t.slice(2));
      else if (mode === 'refs') refEntries.push(t.slice(2));
...
const disclaimer = [
  new Paragraph({ pageBreakBefore: true, spacing: { after: 300 }, children: [new TextRun({ text: 'POLICY PAPER SERIES', bold: true, size: 24, color: BLUE })] }),
  bodyPara('This policy paper presents research and analysis for discussion and comment. The opinions expressed and the arguments employed herein are those of the author(s) alone and do not necessarily reflect the official views of any institution with which they are affiliated.'),
  bodyPara('The paper follows the structure, citation conventions and presentational style of the OECD Policy Papers series for ease of comparison with that literature. It is not an OECD publication, has not been reviewed or endorsed by the OECD, and should not be reported as representing the views of the OECD or of its member countries.'),
  bodyPara('This document, as well as any data included herein, is without prejudice to the status of or sovereignty over any territory, to the delimitation of international frontiers and boundaries and to the name of any territory, city or area. [Standard series disclaimer reproduced for conformity with the OECD Policy Papers presentation; to be confirmed against the internal publication standard before circulation.]'),
...
const acknowledgements = [
  ...
  ...(ack.length ? ack.map(p => bodyPara(p, { run: { italics: true, color: '595959' } })) : [bodyPara('[Acknowledgements to be added before circulation.]', { run: { italics: true, color: '595959' } })]),
];

const abstractBlock = [
  new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: [new TextRun('Abstract')] }),
  ...abstract.map(p => bodyPara(p)),
  ...kwLines.map(l => bodyPara(l, { run: { italics: true, size: 20, color: '595959' }, after: 60 })),
];

const abbrBlock = [
  new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: [new TextRun('Abbreviations and acronyms')] }),
  ...abbr.map(t => new Paragraph({ children: inline(t), spacing: { after: 80 } })),
];

const execBlock = [
  new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: [new TextRun('Executive summary')] }),
  ...exec_.map(p => bodyPara(p)),
];

const notesBlock = noteDefs.length ? [
  new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: [new TextRun('Notes')] }),
  ...noteDefs.map(d => new Paragraph({ children: [new TextRun({ text: d.n, superScript: true }), new TextRun({ text: ' ' }), ...inline(d.text, { size: 20 })], spacing: { after: 120 } })),
] : [];
...
function takeawaysBox() {
  ...
  const children = [
    new Paragraph({ children: [new TextRun({ text: 'Key messages', bold: true, color: BLUE, size: 28 })], spacing: { after: 200 } }),
...
const footerText = 'FORMER TEACHERS: BARRIERS, INCENTIVES AND POLICY LESSONS FOR RE-ENTRY — POLICY PAPER, AUGUST 2026';
...
Packer.toBuffer(doc).then(buf => {
  const out = path.join(DIR, 'Former_Teachers_Policy_Paper_v6.docx');
