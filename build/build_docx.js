...
function buildTable(rows) {
  ...
  const first = nCols > 2 ? Math.round(PAGE_W * 0.34) : Math.round(PAGE_W / nCols);
  const rest = Math.floor((PAGE_W - first) / (nCols - 1 || 1));
  const widths = Array.from({ length: nCols }, (_, i) => (i === 0 ? first : rest));
  ...
  const mkRow = (cells, isHeader, isLast) => new TableRow({
    ...
    children: cells.map((c, i) => new TableCell({
      ...
      children: [new Paragraph({
        children: inline(c, { size: 19, bold: isHeader || undefined }),
  ...
  const flush = () => {
    ...
    if (/^\*\*Box [\d.]+/.test(text)) {
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: t, bold: true, color: BLUE, size: 21 })],
        spacing: { after: 140 },
      }));
      boxList.push(t);
    } else if (/^(Source|Note):/.test(text)) {
      children.push(new Paragraph({ children: inline(text, { size: 17 }), spacing: { after: 40 } }));
    } else {
      children.push(new Paragraph({
...
const tocBlock = [
  ...
  ...tableList.map(c => new Paragraph({ children: [new TextRun({ text: c, size: 20 })], spacing: { after: 60 } })),
...
for (const b of bodyBlocks) {
  if (b.type === 'h1') bodyChildren.push(new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: inline(b.text) }));
    ...
    bodyChildren.push(new Paragraph({ children: [new TextRun({ text: b.caption, bold: true, color: BLUE, size: 21 })], spacing: { before: 200, after: 120 }, keepNext: true }));
...
const tocBoxes = boxList.map(c => new Paragraph({ children: [new TextRun({ text: c, size: 20 })], spacing: { after: 60 } }));
...
const doc = new Document({
  ...
  styles: {
    default: {
      document: { run: { font: FONT, size: 21, color: '000000' } },
    },
    paragraphStyles: [
